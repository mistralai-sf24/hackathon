...
def interleave_list(l1: Sequence[torch.Tensor], l2: Sequence[torch.Tensor]):
...
class CacheView:
    ...
    def interleave_kv(
        ...
        xk_list = torch.split(xk, self.metadata.seqlens)
        xv_list = torch.split(xv, self.metadata.seqlens)
        assert len(xk_list) == len(
            self.kv_seqlens
        ), f"Batch size is {len(self.kv_seqlens)}, got {len(xk_list)}"
        ...
        interleaved_k = interleave_list(cache_k, xk_list)
        interleaved_v = interleave_list(cache_v, xv_list)
